...
class ClientThread(threading.Thread, Configuration):
    ...
    def run(self):
        ...
        # Get lock to synchronize threads
        # threadLock.acquire()
        clientSide(self.REMOTE_IP, self.REMOTE_PORT, self.stream, self.CHUNK, self.queue)
        # Free lock to release next thread
        # threadLock.release()
...
def clientSide(ip, port, stream, chunk, queue):
    serverIP = ip[0]
    serverPort = port
    ...
    while True:
        ...
        queue.put(chunk)
        clientSocket.sendto(queue.get(), (serverIP, serverPort))
        mx = audioop.max(message, 2)
        # print(mx)
